...
- Pydantic v2 for all configs: inherit `BaseConfig`, use `ConfigField` (not `Field`). UI-presentation hints (`advanced`/`hidden`/conditional visibility) live in client's overlay infra, not on the schema.
