...
def test_uniform_mutation_generator_register():
    ...
    gen = UniformMutationGenerator(sequence_length=seq_len, sequence_type=seq_type)
    predefined_seq = ProgramSequence(
        sequence="A" * seq_len, sequence_type=SequenceType.RNA
    )
...
def test_uniform_mutation_generator_sample():
    ...
    short_gen = UniformMutationGenerator(
        sequence_length=1, sequence_type=SequenceType.DNA
    )
    short_gen.register()
    ...
    long_gen = UniformMutationGenerator(
        sequence_length=10000, sequence_type=SequenceType.DNA
    )
    long_gen.register()
...
def test_uniform_mutation_generator_all_types_and_errors():
    ...
    try:
        gen = UniformMutationGenerator(
            sequence_length=0, sequence_type=SequenceType.DNA
        )
...
def test_program_mcmc_generator_score_energy():
    ...
    assert (
        abs(mcmc_gen.score_energy()[0] - expected_score_high) < 1e-9
    )  # Multiplicative
    ...
    assert (
        abs(mcmc_gen.score_energy()[0] - (0.5 * expected_score_high)) < 1e-9
    )  # Multiplicative
    assert (
        abs(mcmc_gen.score_energy("add")[0] - (0.5 * expected_score_high)) < 1e-9
    )  # Additive
...
def test_program_mcmc_generator_no_constraints_and_convergence():
    ...
    proposal_gen = UniformMutationGenerator(
        sequence_length=20, sequence_type=SequenceType.DNA
    )
    sequence_batch = proposal_gen.register()[0]
...
def test_program_mcmc_generator_error_handling_and_state():
    ...
    proposal_gen = UniformMutationGenerator(
        sequence_length=10, sequence_type=SequenceType.DNA
    )
    sequence_batch = proposal_gen.register()[0]
...
def mock_constraint_with_caching(
    input_sequence: ProgramSequence, config: dict
) -> float:
