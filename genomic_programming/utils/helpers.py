...
def calculate_segmasker_score(seq: str, segmasker_path: str = 'segmasker') -> float:
    """
    Calculate the fraction of low-complexity regions in a sequence using segmasker.
    
    Segmasker is part of the NCBI BLAST+ suite and identifies low-complexity regions
    in protein sequences that may cause spurious matches.
    
    Args:
        seq: Protein sequence to analyze.
        segmasker_path: Path or command name for segmasker executable.
        
    Returns:
        Fraction of sequence that is low-complexity (0.0 to 1.0).
        
    Raises:
        SystemExit: If segmasker installation is not found.
        ValueError: If the sequence is empty.
        Exception: If segmasker fails to run.
        
    Example:
        >>> score = calculate_segmasker_score("AAAAAAAAAAA")  # High complexity
        >>> score = calculate_segmasker_score("MVLSPADKTN")  # Normal
    """
    # Check segmasker installation first
    try:
        ...
        print("  or visit: https://ftp.ncbi.nlm.nih.gov/blast/executables/blast+/LATEST/")
        raise SystemExit(1)
    ...
    try:
        ...
        if result.returncode != 0:
            raise Exception(f"Segmasker failed with return code {result.returncode}")
        
        low_complexity_count = result.stdout.count("X")
        return low_complexity_count / total_length
        
    except Exception as e:
        raise Exception(f"Error running segmasker: {str(e)}")
    finally:
        try:
            os.remove(tmp_path)
        except Exception:
